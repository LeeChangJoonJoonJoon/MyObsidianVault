...
----
#Server #수업 #CSharp #listen

## 개요
앞서 서버를 가동키 위한 객체를 초기화하는 `initialize`를 배웠다.
이제 이 준비된 객체를 수신하는 `listen`에 대해 배워볼 것이다.
오늘 다루는 전체 코드는 [[Today's Lecture 6 - initialize]]에 올라가 있으니 참고.
...
## 내용
`CNetworkService`는 `CListener` 클래스로부터 객체를 만들어서 송수신을 한다.
`CListener`는 앞서 나온 것과 같이 구성돼 있다.

`do_listen` 메서드 내에 있는 다음 코드는, 비동기 통신 과정에서 통신이 왔다는 것을 알려주는 신호기이다.
```C#
this.flow_control_event = new AutoResetEvent(false);
```

이때 `AutoResetEvent` 클래스에 `false` 값을 준 이유는, 이것이 아직 신호가 생기지 않았다는 식으로 초기화하기 위함이다.
...
만약 비동기 통신을 진행하던 도중에 클라이언트가 접속을 할 경우 `false` 값을 가지게 되고, 이 값을 인식하여 다음과 같이 신호를 거치지 않고 직접 종료처리를 한다.
```C#
if (!pending)  
{  
	on_accept_completed(null, this.accept_args);  
